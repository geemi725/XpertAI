...
def explain_shap(df_init, model_path, top_k, savedir=None,
                 classifier=False, save_data=True):
    if savedir is None:
        savedir = './data'
...
def explain_lime(df_init, model_path, model_type, top_k=2,
                 savedir=None, save_data=True):
    weights = []
    num_samples = 500
    if savedir is None:
        savedir = './data'
