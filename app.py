...
with st.sidebar:
    ...
    mode_type = st.radio(
        "1. Select the model type", [
            "Regressor", "Classifier"], captions=[
            "For predicting continuous values", "For predicting discreet labels"])
    ...
    XAI_tool = st.radio("2. What's your favorite XAI method?",
                        ["SHAP", "LIME", "Both"])
    top_k = st.slider(
        '3. Select the max number of features to be explained!',
        0,
        10,
        1)
    ...
    lit_files = st.file_uploader(
        "Upload your literature library here (Suggested):",
    ...
    max_papers = st.number_input(
        'Maximum number of papers to download from arxiv.org',
        key=int,
        value=10)
        ...
        if observation is not None:
            ...
            if arxiv_keywords is None and lit_files is None:
                ...
                st.download_button("Download the explanation!",
                                   data=explanation)
                f = open(
                    "./data/figs/structure_function_relationship.txt", 'w+')
            ...
            with open('./data/figs.zip', 'rb') as f:
                st.download_button(
                    'Download the explanation and figures',
                    f,
